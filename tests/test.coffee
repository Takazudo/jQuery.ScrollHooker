(($, window, document) ->
  ...
  # reset something everytime
  QUnit.testDone ->

  # start testing

  test 'ns.normalizeNum', ->
    run = (str, expected) ->
  ...
  test 'ns.normalizeNum cutDecimal', ->
    run = (str, expected) ->
  ...
  test 'ns.parseNum', ->
    run = (str, expected) ->
      res = ns.parseNum str
      equal res, expected, "#{str} - #{res}"
      equal ($.type res), 'number', 'result was number'
    run '100px', 100
    run '-100px', -100
    run '100%', 100
    run '-100%', -100
  ...
  test 'ns.testValsCompatibility', ->
    run = (val1, val2, expected) ->
      res = ns.testValsCompatibility val1, val2
      equal res, expected, "#{val1} and #{val2} : #{expected}"
